    ...
    <section id="hero" className="min-h-screen flex flex-col justify-center items-center px-6 pt-20 overflow-hidden relative">
      ...
      <div className="max-w-7xl w-full text-center space-y-8 relative z-10">
        <div className="space-y-4" style={{ transform: `translateY(${scrollY * 0.2}px)` }}>
          <h1 className="text-5xl md:text-7xl lg:text-8xl font-extrabold text-white tracking-tighter">{portfolioData.name}</h1>
          <p className="font-tech text-neutral-500 tracking-[0.4em] text-sm md:text-base uppercase">{portfolioData.role}</p>
        </div>
        <div className="flex items-center justify-center gap-4 text-neutral-400 py-6" style={{ transform: `translateY(${scrollY * 0.15}px)` }}>
          ...
          <span className="text-xs tracking-widest uppercase">{portfolioData.location}</span>
        </div>
        <div className="flex flex-col md:flex-row gap-4 justify-center items-center pt-4" style={{ transform: `translateY(${scrollY * 0.1}px)` }}>
          <a href="#projects" onClick={(e) => handleScrollTo(e, 'projects')} className="w-48 py-4 bg-white text-black font-semibold text-sm hover:bg-neutral-200 transition-all text-center rounded-full shadow-lg shadow-white/5">{t.heroViewProjects}</a>
          <a href="#about" onClick={(e) => handleScrollTo(e, 'about')} className="w-48 py-4 border border-white/20 text-white font-semibold text-sm hover:bg-white/5 transition-all text-center rounded-full">{t.heroAboutMe}</a>
        </div>
        <div className="flex justify-center gap-8 pt-12" style={{ transform: `translateY(${scrollY * 0.05}px)` }}>
          {portfolioData.socials.map((social) => (social.platform !== 'Email' && (

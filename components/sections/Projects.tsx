          ...
          {PORTFOLIO_DATA.projects.map((project, index) => (
            <button key={project.id} onClick={() => setSelectedProject(project)} className={`group text-left w-full border border-basalt bg-neutral-900/30 overflow-hidden hover:-translate-y-2 hover:border-white/20 transition-all duration-700 rounded-3xl reveal ${isVisible ? 'active' : ''}`} style={{ transitionDelay: `${index * 150}ms` }}>
              <div className="aspect-video bg-neutral-800 overflow-hidden relative">
                ...
                <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-500 z-20"><div className="bg-white/10 backdrop-blur-md px-6 py-2 rounded-full flex items-center gap-2"><span className="text-white text-xs font-bold uppercase tracking-widest">Ver Detalhes</span><Plus className="w-4 h-4 text-white" /></div></div>
              </div>

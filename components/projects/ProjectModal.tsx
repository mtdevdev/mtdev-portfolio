...
import { X, Github, Cpu, Users, Calendar, Gamepad2, FileCode } from 'lucide-react';
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
      ...
      <div className="relative w-full max-w-6xl max-h-[90vh] bg-[#0f0f11] border border-white/10 rounded-3xl overflow-y-auto flex flex-col animate-[fadeInUp_0.4s_ease-out]">
        <div className="sticky top-0 z-10 flex items-center justify-between px-6 py-4 bg-[#0f0f11]/90 backdrop-blur-md border-b border-white/5">
          <div className="flex items-center gap-4">
            <h2 className="text-xl md:text-2xl font-bold text-white tracking-tight">{project.title}</h2>
            <span className="px-3 py-1 bg-white/5 border border-white/10 rounded-full text-[10px] font-tech uppercase tracking-widest text-emerald-400">{project.engine}</span>
          </div>
        ...
        <div className="p-6 md:p-10 grid grid-cols-1 lg:grid-cols-12 gap-10">
          <div className="lg:col-span-8 space-y-10">
            <div className="aspect-video w-full bg-neutral-800 rounded-2xl border border-white/5 overflow-hidden relative group">
            ...
            <div className="space-y-4">
              <h3 className="text-lg font-tech text-white uppercase tracking-widest flex items-center gap-2"><span className="w-1 h-4 bg-emerald-500 rounded-full"></span> Sobre o Projeto</h3>
            ...
            {project.codeSnippet && (
              <div className="space-y-4">
                ...
                <div className="rounded-xl overflow-hidden border border-white/10 bg-[#1e1e1e] text-sm">
                  <div className="flex items-center justify-between px-4 py-2 bg-[#252526] border-b border-white/5">
                    ...
                    <span className="text-[10px] uppercase font-bold text-emerald-500">{project.codeSnippet.language}</span>
                  </div>
          ...
          <div className="lg:col-span-4 space-y-8">
            <div className="flex flex-col gap-3 p-6 bg-neutral-900/50 rounded-2xl border border-white/5">
              {project.isAvailable && project.link && (<a href={project.link} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 w-full py-4 bg-white text-black font-bold text-sm uppercase tracking-wider rounded-xl transition-all"><Gamepad2 className="w-4 h-4" /> Jogar Agora</a>)}
              {project.repoLink && (<a href={project.repoLink} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 w-full py-4 border border-white/10 text-white font-bold text-sm uppercase tracking-wider rounded-xl transition-all"><Github className="w-4 h-4" /> Ver Código</a>)}
            </div>
            <div className="space-y-6">
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap gap-2">{project.techHighlights.map((tech, idx) => (<span key={idx} className="px-3 py-1.5 bg-emerald-950/30 border border-emerald-500/20 text-emerald-400 text-xs rounded-lg">{tech}</span>))}</div>
              </div>
